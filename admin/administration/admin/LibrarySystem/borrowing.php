				...
				<form method = "POST" action = "borrow.php" enctype = "multipart/form-data">
					<div class = "form-group pull-left">	
						...
						<select name = "student_no" id = "student">
							...
							<?php
								$qborrow = $conn->query("SELECT * FROM `student` ORDER BY `lastname`") or die(mysqli_error($conn));
					...
					<table id = "table" class = "table table-bordered">
						...
						<tbody>
							<?php
								$q_book = $conn->query("SELECT * FROM `book`") or die(mysqli_error($conn));
								while($f_book = $q_book->fetch_array()){
								$q_borrow = $conn->query("SELECT SUM(qty) as total FROM `borrowing` WHERE `book_id` = '$f_book[book_id]' && `status` = 'Borrowed'") or die(mysqli_error($conn));
