...
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
   ...
   $connect = mysqli_connect("localhost", "root", ""); // Establishing Connection with Server
		...
		$check = mysqli_query("SELECT * FROM slogin WHERE USN='".$USN."'") or die("Check Query");
 		if(mysqli_num_rows($check) == 0) 
 		{
  			if($repassword == $password)
  			{
    			if($query = mysqli_query("INSERT INTO slogin(Name,USN,PASSWORD,Email,Question,Answer) VALUES ('$Name','$USN','$password','$Email','$Question','$Answer')"))
    			{
