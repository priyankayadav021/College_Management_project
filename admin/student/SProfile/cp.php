...
mysqli_select_db($connect,"placement"); // Selecting Database from Server
	...
	if($Password && $repassword && $cur) 
	{
		...
		{
			$sql = mysqli_query($connect,"SELECT * FROM `placement`.`slogin` WHERE `USN`='$Username'");
			if(mysqli_num_rows($sql) == 1)
			{
				...
				if($cur == $dbpassword)
				{
					if($query = mysqli_query($connect,"UPDATE `placement`.`slogin` SET `PASSWORD` = '$Password' WHERE `slogin`.`USN` = '$Username'"))
					{
