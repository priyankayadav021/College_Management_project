...
$RESULT=mysqli_query($con,"SELECT count(Attendence) from updatedrive where Attendence=1 AND YEAR(Date)=YEAR(NOW())");
$data=mysqli_fetch_assoc($RESULT);
echo "<br><BR>Number of Students Attended In This Year&nbsp:&nbsp";
echo $data['count(Attendence)'];
$RESULT=mysqli_query($con,"SELECT count(Placed) from updatedrive where Placed=1 AND YEAR(Date)=YEAR(NOW())");
$data=mysqli_fetch_assoc($RESULT);
