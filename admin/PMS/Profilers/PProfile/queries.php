...
$RESULT=mysqli_query($conn,"SELECT DISTINCT count(CompanyName) from addpdrive where PVenue LIKE '%CIT%' AND YEAR(Date)=YEAR(NOW())");
$data=mysqli_fetch_assoc($RESULT);
echo "<br><br><h3>Companies In Our Campus In This Year&nbsp:&nbsp";
echo $data['count(CompanyName)'];
$RESULT=mysqli_query($conn,"SELECT count(Attendence) from updatedrive where Attendence=1 AND YEAR(Date)=YEAR(NOW())");
$data=mysqli_fetch_assoc($RESULT);
echo "<br><BR>Number of Students Attended In This Year&nbsp:&nbsp";
echo $data['count(Attendence)'];
$RESULT=mysqli_query($conn,"SELECT count(Placed) from updatedrive where Placed=1 AND YEAR(Date)=YEAR(NOW())");
$data=mysqli_fetch_assoc($RESULT);
